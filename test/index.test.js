...
test("Handle errors with passed callback method", async () => {
...
test("Call callback function with object containing html, css and uncritical props", () => {
...
test("Reject with ConfigError on invalid config", async () => {
  await expect(generate({})).rejects.toThrow(ConfigError);
});

test("Throws FileNotFound error on missing file", async () => {
  await expect(generate({ src: "not-found.html" })).rejects.toThrow(FileNotFoundError);
});

test("Throws NoCssError error on empty styles in strict mode", async () => {
  await expect(
    generate({ src: path.join(__dirname, "fixtures/error.html"), strict: true }),
  ).rejects.toThrow(NoCssError);
});

test("Emit error on streamed file", () => {
  return new Promise((resolve, reject) => {
    ...
    fs.createReadStream(fakeFilePath)
      .pipe(vinylStream())
      .pipe(critical)
      .on("data", () => reject(new Error("Should not emit data")))
      .on("error", ({ message }) => {
...
test("Throws PluginError on critical.stream error", () => {
  return new Promise((resolve, reject) => {
...
test("Support vinyl buffer streams and return critical css vinyl", () => {
  return new Promise((resolve, reject) => {
    const critical = stream({ base: path.join(__dirname, "fixtures") });
...
test("Support vinyl buffer streams and returns html vinyl with inlined css", () => {
  return new Promise((resolve, reject) => {
...
test("Return empty vinyl on empty vinyl", () => {
  return new Promise((resolve, reject) => {
    const critical = stream({ base: path.join(__dirname, "fixtures") });
...
test("#192 - include option - stream", () => {
  return new Promise((resolve, reject) => {
...
test("should generate multi-dimension critical-path CSS in stream mode", () => {
  return new Promise((resolve, reject) => {
