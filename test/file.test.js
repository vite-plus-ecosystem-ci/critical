...
test("Normalize paths", () => {
  ...
  expect(normalizePath(`http://localhost:${port}/styles/main.css`)).toBe(
    `http://localhost:${port}/styles/main.css`,
  );
...
test("Error for file not found", async () => {
  await expect(vinylize({ filenpath: "non-existant-file.html" })).rejects.toThrow(
    FileNotFoundError,
  );
});
...
test("resolve error", async () => {
  const paths = [__dirname, `http://localhost:${port}`];
  await expect(resolve("non-existant-file.html", paths)).rejects.toThrow(FileNotFoundError);
});
...
test("Append stylesheets to vinyl", async () => {
  ...
  const vinyls = await Promise.all(
    files.map((f) => vinylize({ filepath: path.join(__dirname, f) })),
  );
